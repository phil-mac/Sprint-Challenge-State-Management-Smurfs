...
- [ ] What problem does the context API help solve?
  ...
  Prop drilling, which gets overly complicated with lots of children being passes dependencies

- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

  The store maintains all the state of an application, hence being known as the single source of truth. The reducer dictates how changes to state will be made for a given action. An action passes the type of change and any parameters (payload) to the reducer. 

- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?

  Application state is accessible anywhere in the app, which is useful when you have multiple components using the state. Component state is accessible just on a single component, which is good form controll or other UI usecases specific to a single component.

- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

  Redux-thunk is a middleware library that allows your redux actions to return a function instead of an object, which allows for modification of the action.

- [ ] What is your favorite state management system you've learned and this sprint? Please explain why!

  Redux with hooks is convenient, since you can carefully define the state and actions to modify state outside of your components, then with redux hooks you can easily reference either elements in state with useSelector or an action to modify state with useDispatch.
